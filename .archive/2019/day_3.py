    ...
    def coordinateContainsWire(self, x, y, wire) -> Boolean:
        for segment in wire.wire_segments:
            if segment.isHorizontalSegment:
                if (
                    segment.startCoordinate.x <= x <= segment.endCoordinate.x
                    and segment.startCoordinate.y == y
                ):
                    return True
            else:
                if (
                    segment.startCoordinate.y <= y <= segment.endCoordinate.y
                    and segment.startCoordinate.x == x
                ):
                    return True
        return False
        ...
        for pair in dataAsListOfPairedStrings:
            ...
            if currentDirection == "R":
                newEndCoord = Coordinate(
                    (newStartCoord.x + currentMagnitude), newStartCoord.y
                )
            ...
            if currentDirection == "U":
                newEndCoord = Coordinate(
                    newStartCoord.x, (newStartCoord.y + currentMagnitude)
                )
...
class WireSegment:
    ...
    def normalizeCoordinates(self):
        if self.isHorizontalSegment and self.startCoordinate.x > self.endCoordinate.x:
            ...
            not self.isHorizontalSegment
            and self.startCoordinate.y > self.endCoordinate.y
        ):
            temp = self.endCoordinate
            self.endCoordinate = self.startCoordinate
            self.startCoordinate = temp
