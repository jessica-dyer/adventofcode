    ...
    def run_intcode_program(self):
        for num in range(0, len(self.intcode_program.input), 4):
            ...
            if current_opcode == 1:
                try:
                    addition = (
                        self.intcode_program.input[current_instructions[1]]
                        + self.intcode_program.input[current_instructions[2]]
                    )
    ...
    for noun in range(1, len(input)):
        for verb in range(1, len(input)):
            ...
            if intcode_computer.intcode_program.input[0] == 19690720:
                print(
                    "Winner! noun: ",
                    intcode_computer.custom_noun,
                    "verb: ",
                    intcode_computer.custom_verb,
                )
