...
with open("2019/day_3_input.txt") as data:
    data = data.readlines()

directions = dict(zip("UDRL", map(np.array, ([0, 1], [0, -1], [1, 0], [-1, 0]))))

wires = [
    [(directions[d], int("".join(num))) for d, *num in wire.split(",")] for wire in data
]
...
for wire in wires:
    ...
    for direction, number in wire:
        visited.extend(
            tuple(current_location + i * direction) for i in range(1, number + 1)
        )
...
print(
    min(sum(wire.index(location) for wire in visits) for location in intersections)
)  # Part 2
