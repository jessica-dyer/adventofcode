    ...
    for bag_rule in bag_rules:
        if bag_rule.bag_color == bag_color:
            for key in bag_rule.can_contain:
                num_bags_contained = multiplier * find_number_of_bags(
                    key, bag_rule.can_contain[key]
                )
...
def number_of_bags_within(outer_bag_color: str):
    ...
    for inner_bag_color in outer_bags_rule.can_contain:
        ...
        total_inner_bags += number_of_inner_bag + (
            bags_inside_each_inner_bag * number_of_inner_bag
        )
...
def get_rule_for_bag_color(bag_color: str):
    return next(
        (bag_rule for bag_rule in bag_rules if bag_rule.bag_color == bag_color), None
    )
