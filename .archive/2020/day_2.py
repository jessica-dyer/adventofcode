...
def parser(string):
    password_dictionary = {"min": 0, "max": 0, "letter": "", "password": ""}
...
def isValidPassword(password_dictionary):
    ...
    return (
        password_dictionary["min"]
        <= password_char_count[current_letter]
        <= password_dictionary["max"]
    )
...
def isValidPasswordPosition(password_dictionary):
    ...
    one_is_valid = (index_one_is_valid and not index_two_is_valid) or (
        not index_one_is_valid and index_two_is_valid
    )
