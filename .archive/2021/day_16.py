...
class OperatorPacket(AbstractPacket):
    def __init__(
        self, version: int, type: int, array_of_subpackets: list, original_bit_length
    ):
...
class PacketFactory:
    ...
    def build_unknown_packet(self, remaining_binary_string):
        ...
        if type == 4:
            new_packet = self.build_literal_packet(
                version, type, remaining_binary_string
            )
        else:
            new_packet = self.build_operator_packet(
                version, type, remaining_binary_string
            )
        return new_packet
    ...
    def build_operator_packet(self, version, type, remaining_binary_string):
        ...
        remaining_binary_string = remaining_binary_string[
            1 : len(remaining_binary_string)
        ]
